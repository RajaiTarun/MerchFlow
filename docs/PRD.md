...
| **The System Administrator (Super Admin)** | Maintain global platform stability, govern platform access, and resolve edge-case transaction or inventory disputes. | Lack of centralized oversight, rogue club listings, database connection pool exhaustion, and message queue backlogs. | Universal root access; can onboard/deactivate clubs, suspend users, override order states, and monitor system health endpoints. |
